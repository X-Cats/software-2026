package frc.robot.subsystems.HoodKicker;

public class HoodKickerConstants {
  // TODO get actual motor id
  public static final int HOOD_MOTOR_ID = 0;
  public static final int HOOD_MOTOR_VOLTAGE = 1;
  public static final double HOOD_MOTOR_REDUCTION = 1;
  public static final double HOOD_MOTOR_CURRENT_LIMIT = 1;

  public static final int  KICKER_MOTOR_ID = 1;
  public static final int KICKER_MOTOR_VOLTAGE = 1;
  public static final double KICKER_MOTOR_REDUCTION = 1;
  public static final double KICKER_MOTOR_CURRENT_LIMIT = 1;
}
